...
This is the first release of the maintained fork at
<https://github.com/walber-vaz/nginx-language-server>. It is distributed
through GitHub releases, not PyPI.

### Added

- Support for Python 3.13 and 3.14
- Diagnostics: syntax errors, unknown directives and directives used in the
  wrong context (`initializationOptions.diagnostics` to configure them)
- Outline (document symbols)
- Go to definition on `include` and from `*_pass` to `upstream`, and
  clickable `include` links
- Formatting
- Context-aware snippets
- Hover shows a link to nginx.org and marks NGINX Plus features
- Directive data for nginx 1.31, including HTTP/3, QUIC, ACME, OIDC, mgmt
  and OpenTelemetry modules, generated by `scripts/update_nginx_data.py`

### Changed

- New error tolerant parser replaces crossplane: completion and hover work
  while a block or directive is still being typed
- pygls 2 and lsprotocol 2025; pydantic is no longer a dependency
- Project tooling moved to uv and ruff

### Fixed

- Hover on variables (`$host`, `$arg_*`, `$http_*`, ...)
- `stream` and `mail` blocks were offered and documented as `http`
- Several directives on one line hid each other
- Variables were offered where only directives can go

### Removed

- Support for Python 3.9
- Obsolete SPDY directives
- The Node.js data scripts
